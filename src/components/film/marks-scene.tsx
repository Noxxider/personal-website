...
function Tile({ label, index }: { label: string; index: number }) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    g.position.set(
      side * 0.7 + drift.current.x + Math.sin(t * 0.6) * 0.03,
      Math.cos(t * 0.8) * 0.04 + drift.current.y - (1 - enter) * 0.4 + leave * 0.9,
      0,
    );
    g.rotation.set(
      Math.sin(t * 0.5) * 0.14 - drift.current.y * 0.7,
      Math.sin(t * 0.4) * 0.22 + drift.current.x * 0.9 + side * 0.18,
      Math.sin(t * 0.3) * 0.05,
    );
  ...
  return (
    <group ref={group}>
      <RoundedBox args={[TILE, TILE, 0.14]} radius={0.12} smoothness={8}>
        <meshStandardMaterial color={face} roughness={0.28} metalness={0.25} />
      </RoundedBox>
